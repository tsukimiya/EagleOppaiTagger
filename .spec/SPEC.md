...
### 15.10 自動停止時のエラー診断（Phase 10.2）

連続エラーで自動停止した際、ユーザーが UI 表示だけで原因を自己診断できなければならない（Phase 10.1 の実機調査で、停止メッセージが汎用文のみで原因特定に DevTools のログが必要だった問題への対応）。

- `auto-tagger.js` は失敗した処理の履歴 `errorHistory`（上限10件のリングバッファ: `{at, fileName, message}`）を保持し、`getState()` で公開する。`start()` 時にリセットする
- 連続エラーが閾値に到達した際の `onWarning` ペイロードには、停止理由に加えて `lastError`（直近エラーメッセージ）と `errorHistory` を含める
- 1回の自動停止で `onWarning` の発火は1回とする（`tick()` 内で警告後に `stop()` を呼ぶ場合、`stop()` 側は再発火しない）
- UI の停止メッセージには `lastError` を含める。`errorHistory` 全文は `title` 属性（ホバー表示）と「詳細コピー」ボタン（クリップボードへエラー履歴 + 自動モード設定を書き出し）で取得可能とする
