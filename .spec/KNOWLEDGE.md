...
## Phase 10.4 — WebP 等の未対応画像形式のデコード対応（2026-07-23）

### 根因と影響範囲

- `Unsupported MIME type: image/webp` は **Jimp 0.22.12 のコア**が投げる（Jimp は WebP 未登録・Issue jimp-dev/jimp#144 由来の長期未対応）。拡張子ではなくマジックバイトで判定するため `.png.webp` の二重拡張子でも正しく webp と認識されて失敗する
- 影響は**ローカル推論と、サーバ失敗時のローカルフォールバック**のみ。サーバ推論（`inferRemote`）は生ファイルを送出しサーバ側（Python/Pillow）で前処理するため webp でも正常
- エラーは `preprocess.js` の `Jimp.read(filePath)` 起点 → `main.run` の catch → Phase 10.2 のエラー診断 UI で観測された（診断強化が早期発見に寄与）

### 採用した修正（ゼロ依存・外科的）

- `readImage()` を「まず `Jimp.read`（PNG/JPEG/BMP/TIFF/GIF — Python 参照実装との MAE 契約を維持）、失敗時のみ Chromium DOM（`createImageBitmap` + canvas）でデコード → `rgbaToJimp()` で Jimp bitmap 化」の2段に変更。下流の resize/letterbox/normalize は無変更
- DOM デコードは renderer 専用（`isDomDecodeAvailable()` でガード）。Node テストでは DOM がないため、`createImageBitmap`/`document`/`Blob` をスタブ化して配管を検証。実際のデコードは実機検証に委ねる（既存の「実機検証待ち」パターン）
- 副次効果: WebP だけでなく **Chromium が読める形式（AVIF/GIF 等）全般に自動対応**し、次回の「Unsupported MIME type」を予防
- `new Jimp({ data, width, height }, cb)` で RGBA から Jimp を生成できることを実機 Node で確認済み（ピクセル値保持）

### テスト

- `preprocess-webp-test.js`（14 assertions）: DOM スタブが返す RGBA を webp 風ファイル経由で通した `preprocess` 出力が、同ピクセルの PNG を通常経路で通した出力と **pixel_values / padding_mask 完全一致**することを検証（本バグの回帰テスト）。DOM 非利用時は Jimp の元エラーが伝播することも確認

### 配布

- `make-dist.ps1` の src 除外パターンを `phase*-test.js` → `*-test.js` に汎化（`preprocess-webp-test.js` が配布 zip に混入するのを防止。本番ファイルに `-test.js` 終わりは存在しないため安全）

### ADR 候補

- **ADR-14 候補**: Jimp 未対応形式のデコードに「依存パッケージ（sharp / @jimp/wasm-webp 等）を追加せず、Eagle renderer の Chromium DOM デコードにフォールバックする」方式を採用
